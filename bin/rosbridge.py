	...
	try:
		...
		if 'type' in call.keys():
			...
			if (not passfile or (session.authStatus == 'accepted')):
				...
				first = True
				if not receiver in session.publishers.keys():
					session.publishers[receiver] = rospy.Publisher(receiver.encode('ascii','ignore'), ros.msgClassFromTypeString(typ))
				else:
					first = False

				if first:
						sleep(.16) #publisher creation doesn't block
				if (first and msg != {} or msg != []) or not first or typ=="std_msgs/Empty":
			...
			if (not passfile or (session.authStatus == 'accepted') or receiver.find('/rosbridge/challenge') == 0 or receiver.find('/rosbridge/authenticate') == 0):
				if receiver.find('/rosbridge') == 0:
					if (receiver == "/rosbridge/topics"):
						...
						if len(msg) == 3:
							status,handleLatch = sub(session, topic, handleMsgFactory(session, topic), msg[2])
						else:
							status,handleLatch = sub(session, topic, handleMsgFactory(session, topic))
...
if __name__ == "__main__":
	...
	def sub(session, topic, handler, typ = None):
		...
		if not repeat:
			try:
				...
				subscriber = rospy.Subscriber(topic, cls, handler, queue_size=10)
				session.latestLock.acquire()
	...
	VersionChecker(2451).check() #manually updated	
